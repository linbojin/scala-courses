...
object Interaction2 {
  ...
  def availableLayers: Seq[Layer] = {
    ...
    Seq(
      Layer(LayerName.Temperatures, tc, 1975 to 2015),
      Layer(LayerName.Deviations, dc, 1991 to 2015))
  }
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Int]): Signal[Int] = {
    Signal(if (sliderValue() > yearBounds(selectedLayer)().max) yearBounds(selectedLayer)().max
    else if (sliderValue() < yearBounds(selectedLayer)().min) yearBounds(selectedLayer)().min
    else sliderValue())
  }
