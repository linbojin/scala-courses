...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary = columnNames.filter(c => c.startsWith("t01") ||
      c.startsWith("t03") ||
      c.startsWith("t11") ||
      c.startsWith("t1801") || c.startsWith("t1803")
    )
    val working = columnNames.filter(c => c.startsWith("t05") || c.startsWith("t1805"))

    val leisure = columnNames.filter(c => (c.startsWith("t0") || c.startsWith("t1")) &&
      !(c.startsWith("t01") ||
      c.startsWith("t03") ||
      c.startsWith("t11") ||
      c.startsWith("t1801") ||
    ...
    (primary.map(col), working.map(col), leisure.map(col))
  }
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = primaryNeedsColumns.map(_ / 60.0)
      .reduce(_ + _).as("primaryNeeds")
    val workProjection: Column = workColumns.map(_ / 60.0)
      .reduce(_ + _).as("work")
    val otherProjection: Column = otherColumns.map(_ / 60.0)
      .reduce(_ + _).as("other")
    df
